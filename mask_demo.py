...
class YOLOv7Mask():
    def __init__(self,
                 weight_path='yolov7-mask.pt',
                 hyp_cfg="data/hyp.scratch.mask.yaml", onnx=False) -> None:
    ...
    def get_mask(self,
        ...
        # image = image.cuda()
        if self.onnx:
            print(f"infer shape = {image.size()}")
            dummpy = torch.zeros(image.size())
            model_cpu = model.cpu()
            torch.onnx.export(model_cpu, (dummpy, ), 
            'mask.onnx', 
            input_names=["images"],
            output_names=["test", "atten", "box_cls_0", "box_cls_1", "box_cls_2", "bases", "sem"],
            verbose=True, 
            opset_version=12)
        model = model.cuda()
        output = model(image)
