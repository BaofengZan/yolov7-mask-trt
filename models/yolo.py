    ...
    def forward(self, x):
        ...
        for i in range(self.nl):
            if self.attn is not None:
                ...
                bs, _, ny, nx = map(int, attn[i].shape)  # x(bs,2352,20,20) to x(bs,3,20,20,784)
                #attn[i] = attn[i].view(bs, self.na, self.attn, ny, nx).permute(0, 1, 3, 4, 2).contiguous()
                attn_numpy = attn[i].view(bs, self.na, self.attn, ny, nx).permute(0, 1, 3, 4, 2).contiguous()
            if self.mask_iou:
                iou[i] = self.m_iou[i](x[0][i])
            x[0][i] = self.m[i](x[0][i])  # conv
            bs, _, ny, nx = map(int, x[0][i].shape)  # x(bs,255,20,20) to x(bs,3,20,20,85)
            #x[0][i] = x[0][i].view(bs, self.na, self.no, ny, nx).permute(0, 1, 3, 4, 2).contiguous()
            x_0_i = x[0][i].view(bs, self.na, self.no, ny, nx).permute(0, 1, 3, 4, 2).contiguous()
            if self.mask_iou:
                iou[i] = iou[i].view(bs, self.na, ny, nx).contiguous()

            if not self.training:  # inference
                #za.append(attn[i].view(bs, -1, self.attn))
                _,c,h,w,_ = attn_numpy.size()
                za.append(attn_numpy.view(bs, -1, self.attn))
                if self.mask_iou:
                ...
                #y[..., 2:4] = (y[..., 2:4] * 2) ** 2 * self.anchor_grid[i]  # wh
                xy = (y[..., 0:2] * 3. - 1.0 + self.grid[i].to(y.device)) * self.stride[i].to(y.device)  # xy
                anchor_grid = self.anchor_grid[i].clone().view(1, -1, 1, 1, 2)
                wh = (y[..., 2:4] * 2) ** 2 * anchor_grid.to(y.device)  #.view(1, self.na, 1, 1, 2) # wh
        ...
        if not self.training:
            ...
            if self.mask_iou:
                output["mask_iou"] = iou
        output["bbox_and_cls"] = x[0]  # 这个值已经不对了，但是后面没用到，所有不用管。
        output["bases"] = x[1]
        output["sem"] = x[2]
